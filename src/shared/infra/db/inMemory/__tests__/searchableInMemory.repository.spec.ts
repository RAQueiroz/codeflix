...
import { SearchableInMemoryRepository } from '../searchableInMemory.repository';
...
class StubInMemorySearchableRepository extends SearchableInMemoryRepository<
  StubEntity,
  string
> {
  ...
  protected async applyFiltering(
    items: StubEntity[],
    filter: string | null,
  ): Promise<StubEntity[]> {
    if (!filter) {
...
describe('InMemorySearchableRepository Unit Tests', () => {
  ...
  describe('applyFiltering method', () => {
    ...
    it('should filter using a filter param', async () => {
      ...
      const spyFilterMethod = jest.spyOn(items, 'filter' as any);
      let itemsFiltered = await repository['applyFiltering'](items, 'TEST');

      expect(itemsFiltered).toStrictEqual([items[0], items[1]]);
      expect(spyFilterMethod).toHaveBeenCalledTimes(1);

      itemsFiltered = await repository['applyFiltering'](items, '5');
      expect(itemsFiltered).toStrictEqual([items[0], items[1]]);
      expect(spyFilterMethod).toHaveBeenCalledTimes(2);

      itemsFiltered = await repository['applyFiltering'](items, 'no-filter');
      expect(itemsFiltered).toHaveLength(0);
      expect(spyFilterMethod).toHaveBeenCalledTimes(3);
    });
  });

  describe('applySorting method', () => {
    it('should no sort items', async () => {
      ...
      let itemsSorted = await repository['applySorting'](items, null, null);
      expect(itemsSorted).toStrictEqual(items);

      itemsSorted = await repository['applySorting'](items, 'price', 'asc');
      expect(itemsSorted).toStrictEqual(items);
    });
